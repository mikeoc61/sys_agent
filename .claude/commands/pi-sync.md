...
6. If everything passed, run the hardware harness on pibot under both interpreters and report each RESULT line and `[readline]` backend: `ssh -o BatchMode=yes pibot 'cd ~/projects/sys_agent && ~/.local/bin/uv run --quiet --python 3.13 test_hardware.py; python3 test_hardware.py'`.
